...
function importGame(model: PyChessModel): void {
    loadFFish().then(ffish => {
        ...
        if (pgn.indexOf(BRAINKING_SITE) !== -1 && pgn.indexOf(EMBASSY_FEN) !== -1) {
            ...
            // fix starting FEN
            const fenIndex = lines.findIndex(elem => elem.startsWith('[FEN '));
            lines[fenIndex] = `[FEN "${VARIANTS['embassy'].startFen}"]`;

            // Switch short/long castle notation
            lines.forEach((line, idx) => {
                if (idx > fenIndex) lines[idx] = line.replace(/(O-O-O|O-O)/g, (match) => match === 'O-O' ? 'O-O-O' : 'O-O');
            });
        ...
        XHR.onreadystatechange = function() {
            if (this.readyState === 4 && this.status === 200) {
                ...
                if (response['gameId'] !== undefined) {
                    window.location.assign('/' + response['gameId']);
